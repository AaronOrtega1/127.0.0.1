module.exports = async (params) => {
  ...
  const title = await quickAddApi.inputPrompt(
    "Enter the title of the Zettelkasten Note:",
  );
  ...
  const folderPath = "04-Zettelkasten"; // Cambia esto por la ruta de tu carpeta
  const filePath = `${folderPath}/${noteID}.md`;

  // Ruta del template
  const templatePath = "06-Templates/note.md"; // Cambia esto por la ruta de tu template
