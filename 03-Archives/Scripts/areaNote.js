module.exports = async (params) => {
  ...
  const title = await quickAddApi.inputPrompt(
    "Enter the title of the Area Note:",
  );
  ...
  const folderPath = "01-Areas"; // Cambia esto por la ruta de tu carpeta
  const filePath = `${folderPath}/${noteID}.md`;

  // Ruta del template
  const templatePath = "06-Templates/area.md"; // Cambia esto por la ruta de tu template
  ...
  try {
    ...
    const finalContent = templateContent
      .replace(/{{cap_title}}/g, title.split(' ').map(palabra => palabra.charAt(0).toUpperCase() + palabra.slice(1).toLowerCase()).join(' '))
